...
const filter1 = fifaData.filter((element) => {
    if (element.Year === 2014 && element.Stage === 'Final'){
        return element
    }
})
//(a) Home Team name for 2014 world cup final
    console.log('Home team:', filter1[0]['Home Team Name'])
//(b) Away Team name for 2014 world cup final
    console.log('Away team:', filter1[0]['Away Team Name'])
//(c) Home Team goals for 2014 world cup final
    console.log('Home team goals:', filter1[0]['Home Team Goals'])
//(d) Away Team goals for 2014 world cup final
    console.log('Away team goals:', filter1[0]['Away Team Goals'])
//(e) Winner of 2014 world cup final */
    console.log('Winner:', filter1[0]['Win conditions'])
...
function getFinals(fifaData) {
    let teams = fifaData.filter((teamNames) => {
       if(teamNames.Stage === 'Final'){
        return teamNames.Stage
       }
    })
   return teams
}
...
function getYears(fifaData, getFinals) {
    const map1 = fifaData.map((element)=>{
        return element.Year
    });
   return map1
}
...
function getWinners(fifaData,getFinals) {
    const filter1 = getFinals(fifaData).map((winner) => {
...
function getWinnersByYear(data, getFinalsCb, getYearsCb, getWinnersCb) {
   ...
   const yearWins = getYearsCb(getFinalsCb(data));
   const countryWins = getWinnersCb(data, getFinalsCb);
   return countryWins.map((element, index) => `In ${yearWins[index]}, ${element} won the world cup!` )
    }
...
function getAverageGoals(getFinalsCb) {
    const averageGoals = getFinalsCb.reduce((prev, current)=>{
